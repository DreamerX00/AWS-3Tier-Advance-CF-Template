...
import com.backend.aws.service.FileService;
...
public class FileController {
    ...
    public ResponseEntity<byte[]> download(@PathVariable String s3Key) throws IOException {
        FileMetadata metadata = service.findAll().stream()
                .filter(f -> f.getS3Key().equals(s3Key))
                .findFirst()
                .orElseThrow(() -> new RuntimeException("File not found"));

        byte[] data = service.download(s3Key);
